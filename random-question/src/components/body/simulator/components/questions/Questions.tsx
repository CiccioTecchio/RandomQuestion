...
export default function Questions(props:{questions:Array<IQuestion>, qaList:Array<IQuestionAnswered>, updateQAInSimulation:Function, currentQuestionIdx:number, changeQuestion:Function, goToResume:Function}): ReactElement {
  const handleAnsweredQuestion = (value:string, options:TypeQuestion) => {
    const tmp:Array<IQuestionAnswered> = [...props.qaList];
    ...
    switch (options) {
      ...
      case TypeQuestion.Multiple: {
        const idx = props.qaList[currentQuestionIdx].selectedOptions.findIndex((e:string)=> e === value);
  ...
  const disableNext = () => props.qaList[props.currentQuestionIdx].selectedOptions.length === 0 || props.currentQuestionIdx+1 === props.questions.length;
    ...
    <Row className='mt-3'>
      ...
      <Col md={9} >
        <QuestionFrame question={props.questions[props.currentQuestionIdx]} idxQuestion={props.currentQuestionIdx}
          selectedOptions={props.qaList[props.currentQuestionIdx].selectedOptions} updateSelectedOptions={handleAnsweredQuestion}
          isFlagged={props.qaList[props.currentQuestionIdx].isFlagged} updateFlag={handleFlaggedQuestion}></QuestionFrame>
      </Col>
      <Col className='align-self-center d-flex justify-content-end'>
        ...
        </Button>: <SendSimulation lastSelectedOptions={props.qaList[props.currentQuestionIdx].selectedOptions} goToResume={props.goToResume}/>}
      </Col>
    </Row>);
}
